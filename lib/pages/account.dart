                          ...
                          children: [
                            ...
                            SingleChildScrollView(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  children:
                                      List.generate(readLater.length, (index) {
                                    return GestureDetector(
                                        onTap: () {
                                          Navigator.push(
                                              context,
...
class __UserProfileState extends State<_UserProfile> {
  ...
  Future<Map> getUser() async {
    ...
    dbUser = futures[2] as Map;
    pfpImg = futures[3] as String;

    return streamUser.data!;
  }

  Future<Map> getDbUser() async {
    final box = GetStorage();
    final dbUser = await box.read('user');
    return dbUser;
  }
      ...
      children: [
        ...
        Align(
          ...
          child: Padding(
            ...
            child: Text(widget.dbUser['profile']['username'],
                style: AppTextStyle.textStyleBoldMedium),
          ),
        ),
        if (widget.dbUser['profile']['bio'] != "")
          Align(
            alignment: Alignment.centerLeft,
            child: Padding(
              padding: const EdgeInsets.only(left: 8, right: 8, bottom: 8),
              child: Text(
                widget.dbUser['profile']['bio'],
              ),
            ),
          ),
